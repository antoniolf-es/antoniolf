...
final class ImagenBlog
{
    ...
    public function guardar(array $fichero, string $base): bool
    {
        ...
        if ($codigo === UPLOAD_ERR_NO_FILE) {
            $this->error = 'Selecciona una imagen para el artículo.';
        ...
        $ruta = dirname(__DIR__, 2) . '/img/blog/' . $base . '.jpg';
    ...
    public static function borrar(null|string $fichero): void
    {
        if ($fichero === null || $fichero === '') {
            return;
        }

        @unlink(dirname(__DIR__, 2) . '/img/blog/' . $fichero);
    }
